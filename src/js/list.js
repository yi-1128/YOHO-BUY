...
$(function(){
    $(".sideMenu .nLi h3").click(function(){
        if($(this).parent(".nLi").hasClass("on")){
            //改变三角的方向
            $(this).find('.icon-triangle').css('transform','rotate(0deg)');
            //展开
...
$('.supper').hover(
    function(){
        $('dl:last-child').css('display','block')
    },
    function(){
        $('dl:last-child').css('display','none')
    }
)

//滑过大图片出现小图片
$('.good').each(function(i,value){
    $(value).hover(
        function(){
            $(this).find('.good_mark').css('display','block');
        },
        function(){
            $(this).find('.good_mark').css('display','none');
        }
    )
})